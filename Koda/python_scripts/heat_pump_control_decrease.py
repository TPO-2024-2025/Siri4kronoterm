...
TEMP_DECREMENT = 1.0         # Changed from increment to decrement
DEFAULT_TEMP = 22.0  # Default temperature if sensor is unavailable
MIN_TEMPERATURE = 17.0  # Minimum allowed temperature
MAX_TEMPERATURE = 27.0  # Maximum allowed temperature (kept for reference)

try:
    # Log the start of the script
    logger.info("Starting heat pump temperature decrease")  # Updated log message
    ...
    if heat_pump_state is None:
        ...
        if current_state is None:
            ...
            # Calculate new temperature (decrease instead of increase)
            new_temp = current_temp - TEMP_DECREMENT
            
            # Check if new temperature would be below minimum
            if new_temp < MIN_TEMPERATURE:
                logger.warning(f"New temperature {new_temp}°C would be below minimum of {MIN_TEMPERATURE}°C. Limiting to minimum.")
                new_temp = MIN_TEMPERATURE
                message = f"Heat pump temperature limited to minimum {MIN_TEMPERATURE}°C (from {current_temp}°C)"
            else:
                # Normal temperature decrease message
                if is_default:
                    message = f"Heat pump temperature set to {new_temp}°C (decreased from default of {current_temp}°C because sensor was {current_state.state})"
                else:
                    message = f"Heat pump temperature decreased from {current_temp}°C to {new_temp}°C"
